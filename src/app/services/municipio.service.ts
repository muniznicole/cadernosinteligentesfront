...
export class MunicipioService {
    ...
    buscar(id: string): Observable<Municipio> {
        return this.httpClient.get<Municipio>(`${this.baseUrl}/${id}`);
    }

    inserir(municipio: Municipio): Observable<Municipio> {
        const data = {
            nome: municipio.nome,
    ...
    alterar(municipio: Municipio): Observable<Municipio> {
        const data = {
            nome: municipio.nome,
            idEstado: municipio.estado.id
        }
    ...
    deletar(municipio: Municipio): Observable<any> {
