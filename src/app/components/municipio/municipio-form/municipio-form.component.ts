...
export class MunicipioFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.estadoService.buscarTodos().subscribe(data => {
      this.estados = data;
      this.initializeForm();
    });
  }
  ...
  salvar() {
    if (this.formGroup.valid) {
      ...
      if (municipio.id ==null) {
        this.municipioService.inserir(municipio).subscribe({
        ...
        this.municipioService.alterar(municipio).subscribe({
  ...
  excluir() {
    if (this.formGroup.valid) {
      const municipio = this.formGroup.value;
      if (municipio.id != null) {
        this.municipioService.deletar(municipio).subscribe({
          next: () => {
            this.router.navigateByUrl('/municipios');
          },
          error: (err) => {
